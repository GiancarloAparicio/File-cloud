@switch( Str::of($file->original_name)->after('.') )
    ...
    @case('ptt')
    <span
        class="h-8 w-8 bg-yellow-400 pt-2 text-xs text-white text-center align-bottom rounded-lg object-cover ">{{ Str::of($file->original_name)->after('.') }}</span>
    @break

    @case('png' )

    <span
        class="h-8 w-8 bg-green-400 pt-2 text-xs text-white text-center align-bottom rounded-lg object-cover ">{{ Str::of($file->original_name)->after('.') }}</span>
    @break
    ...
    <span
        class="h-8 w-8 bg-blue-400 pt-2 text-xs text-white text-center align-bottom rounded-lg object-cover ">{{ Str::of($file->original_name)->after('.') }}</span>
    @break
